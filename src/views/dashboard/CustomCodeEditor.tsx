...
const highlightCode = (code: string, language: CodeLanguage) => {
	...
	if (language === "css") {
		return escaped
			...
			.replace(/([#.][a-zA-Z0-9_-]+)(?=\s*\{)/g, '<span class="token selector">$1</span>')
			.replace(/([a-z-]+)(?=\s*:)/g, '<span class="token property">$1</span>')
			.replace(/(:\s*)([^;]+)(?=;)/g, '$1<span class="token value">$2</span>');
	}
	if (language === "html") {
		return escaped.replace(/(&lt;\/?)([a-zA-Z0-9:-]+)([^&]*?)(\/?>)/g, (_match, open, tag, attrs, close) => {
...
const CustomCodeEditor: React.FC = () => {
	...
	return <EditorInstance key={codeEditorTarget.widgetId} target={codeEditorTarget} />;
};
			...
			<div className="CustomCodeEditor" role="dialog" aria-modal="true">
				...
				<div className="editor-body">
					<div className="code-shell">
						<div className="line-numbers" ref={lineNumberRef} aria-hidden="true">
							{lines.map((_, index) => (
					...
					<div className="editor-hints">
						...
						<p>
							Press <kbd>Ctrl</kbd> + <kbd>S</kbd> (or <kbd>⌘</kbd> + <kbd>S</kbd>) to save instantly. Use this
							editor for advanced tweaks—changes apply immediately across devices.
						</p>
					</div>
				</div>
			</div>
