...
const Input: FC<Props> = (props) => {
  ...
  const [isDetailsOpen, setIsDetailsOpen] = useState(props.number === 1);
  ...
  const renderSelectedIcon = (): ReactNode => {
    const iconSize = props.iconSize ?? props.customWidth ?? 20;
    const domain = getDomain(urlValue || props.url);

    if (props.icon === "_favicon_duckduckgo" && domain) {
      return (
        <img
          alt=""
    ...
    if (
      (props.icon === "_favicon_google" || props.icon === "_favicon") &&
      domain
    ) {
      return (
        <img
    ...
    if (props.icon === "_favicon_favicone" && domain) {
      return (
        <img
          alt=""
    ...
    if (props.icon === "_custom_iconify" && props.IconString) {
      return (
        <Icon icon={props.IconString} width={iconSize} height={iconSize} />
      );
    }

    if (props.icon === "_custom_ico" && props.IconStringIco) {
      return (
        <img
          alt=""
          src={props.IconStringIco}
          style={{ width: iconSize, height: iconSize }}
        />
      );
    }
    ...
    if (
      props.icon === "_custom_upload" &&
      props.iconCacheKey &&
      props.cache?.[props.iconCacheKey]
    ) {
      const cachedIcon = props.cache[props.iconCacheKey];
      if (cachedIcon.type !== "svg") {
        return (
          <img
            alt=""
            src={cachedIcon.data}
      ...
      return <Icon icon="feather:image" width={iconSize} height={iconSize} />;
    }

    if (props.icon === "_custom_svg") {
      return <Icon icon="feather:image" width={iconSize} height={iconSize} />;
    }

    if (props.icon === "_feather") {
      return (
        <Icon
          icon={
            props.iconifyValue
              ? `${props.iconifyIdentifier ?? "feather:"}${props.iconifyValue}`
              : "feather:bookmark"
          }
          width={iconSize}
          height={iconSize}
    ...
    return <Icon icon="feather:link" width={iconSize} height={iconSize} />;
  };
  ...
  const linkLabel = props.name || props.url || "Link Name";

  return (
    <div className={`LinkInput ${isDetailsOpen ? "is-expanded" : ""}`}>
      <div className="link-row">
        <div className="link-row-icon" aria-hidden="true">
          {renderSelectedIcon()}
        </div>

        <div className="link-row-main">
          {props.number <= 9 && (
            <span className="link-number">{props.number}</span>
          )}
          <input
            type="text"
            value={props.name}
        ...
        <div className="link-row-actions">
          ...
          <button
            type="button"
            className="link-action-button danger"
          ...
          <button
            ...
            title={isDetailsOpen ? "Hide link options" : "Show link options"}
            aria-label={
              isDetailsOpen ? "Hide link options" : "Show link options"
            }
            aria-expanded={isDetailsOpen}
          >
            <Icon
              icon={
                isDetailsOpen ? "feather:chevron-up" : "feather:chevron-down"
              }
            />
          </button>
        </div>
      ...
      {isDetailsOpen && (
        <div className="link-input-details">
          <div className="link-details-panel link-details-panel--main">
            <label>
              <FormattedMessage
          ...
          <div className="link-details-panel link-details-panel--options">
            <label>
              <FormattedMessage
              ...
              <select
                ...
                value={props.icon}
                onChange={(event) =>
                  props.onChange({ icon: event.target.value })
                }
              >
                <option value="">
                  <FormattedMessage
              ...
              <>
                <label>
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      props.onChange({
                        conserveAspectRatio: event.target.checked,
                      })
                    }
                  />
                </label>
